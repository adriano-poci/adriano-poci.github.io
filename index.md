...
<div class="resGrid">
    ...
    <div class="grid-row" style="grid-column: 2; grid-row: 1; text-align: left;">The effect of orientation on models of strong galaxy-galaxy lensing. The figure shows three orientations (left to right) of a known nearby strong-lens galaxy, SNL-1, and the corresponding stellar kinematics (top) and lensing cross sections (bottom). These projections are generating from a Schwarzschild orbit-based dynamical model, fit to the observed stellar kinematics. The result is that simply changing the orientation of the lens galaxy has a substantial impact on the derived cross-section, and may impact subsequent physical inferences. See
